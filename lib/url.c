...
typedef HINSTANCE(WINAPI *ShellExecFcn)(HWND, LPCSTR, LPCSTR, LPCSTR, LPCSTR, int);
ShellExecFcn orgShellExec = 0;

HINSTANCE WINAPI hookShellExec(HWND hwnd, LPCSTR lpOperation, LPCSTR lpFile, LPCSTR lpParameters, LPCSTR lpDirectory, int nShowCmd) {
	
	//if(!strcmp(lpFile, "http://principiagame.com"))
	lpFile = str_replace(lpFile, "principiagame.com", "principiagame.000webhostapp.com");

	if (!strcmp(lpOperation, "open"))
		printf("Opening %s!", lpFile);

	return orgShellExec(hwnd, lpOperation, lpFile, lpParameters, lpDirectory, nShowCmd);
}

int base = 0;

typedef size_t(__cdecl *fwriteFcn)(const char*, size_t, size_t, FILE*);
fwriteFcn orgfwrite = 0;
...
typedef int(__cdecl *fprintfFcn)(FILE*, const char*, ...);
fprintfFcn orgfprintf = 0;

int __cdecl hookfprintf(FILE* file, const char* fmt, ...) {
	
	char buff[256];
	va_list args;
	va_start(args, fmt);
	vsprintf_s(buff, 256, fmt, args);
	va_end(args);

	if (file == *(FILE**)(base + 0x443058))
		printf("[LOG] %s", buff);

	return orgfprintf(file, "%s", buff);
}
...
void Init() {
	...
	printf("principia.exe - %x\n", base);

	printf("Initializing MinHook...\n");
	MH_Initialize();
	
	int* addr = (int*)GetProcAddress(GetModuleHandleA("Shell32.dll"), "ShellExecuteA");
	if (MH_CreateHook(addr, hookShellExec, &orgShellExec) != MH_OK) printf("Failed to hook ShellExecA! Links opened in-game will open the original principia website.\n");

	//printf("Replacing the log file...\n");
	//*(FILE**)(base + 0x443058) = stdout;
	
	int* fwriteaddr = base + 0x18C68;
	if (MH_CreateHook(fwriteaddr, hookfwrite, &orgfwrite) != MH_OK) printf("Failed to hook fwrite! Log will not be shown.\n");
	
	int* fprintfaddr = base + 0x18CA0;
	if (MH_CreateHook(fprintfaddr, hookfprintf, &orgfprintf) != MH_OK) printf("Failed to hook fprintf! Log will not be shown.\n");

	printf("Enabling hooks...\n");
	if (MH_EnableHook(MH_ALL_HOOKS) != MH_OK) printf("Failed no enable hooks!\n");
                     ...
                     va_list param)
{
  ...
  case CURLOPT_URL:
	...
	url = str_replace(url, "principiagame.com", "principiagame.000webhostapp.com");
